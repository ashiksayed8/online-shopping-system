<section class="header">
    <div class="side-menu" id="side-menu" style="background: #254117;color:white;">
        @php
         $category=DB::table('categories')->get();
        @endphp
        <ul>
            @foreach($category as $row)
            <li> <a style=" text-decoration: none; color:white;" href="{{ url('product/category/'.$row->id) }}"> {{ $row->category_name }} <i class="fa fa-angle-right"></a></i>
               <ul> 
                   @php
                     $subcategory=DB::table('subcategories')->where('category_id',$row->id)->get(); 
                   @endphp
                   @foreach($subcategory as $row)
                   <li><a href="{{ url('product/sub_category/'.$row->id) }}">{{ $row->subcategory_name }}</a></li> 
                   @endforeach 
               </ul>
            </li>
            @endforeach
    ...
    @endphp
    <div class="slider" style="background: #2B1B17; color:white;">
